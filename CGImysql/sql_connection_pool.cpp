...
void Connection_Pool::init(string url, string user, string password,
                           string DataBaseName, int port, int maxConnections,
                           int close_log) {
...
int Connection_Pool::GetFreeConnections() { return this->m_FreeConnections; }

// *!* Using RAII to release/get the connection

Connection_RAII::Connection_RAII(MYSQL **SQL,
                                 Connection_Pool *connection_pool) {
...
Connection_RAII::~Connection_RAII() { pool_RAII->ReleaseConnection(con_RAII); }
